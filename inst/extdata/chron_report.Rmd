...
if (nrow(chron_detrended$df_data<1)){
  ...
  plot1 <- plot_all_series(chron_data, text_size = 8, plot_line = 0.5, line_width = 0.5)
  ...
  if(is.null(N.plot.dat) || ncol(N.plot.dat)<2){
      ...
      plot2<-ggplot()+
        ...
        R_dateR_theme(text_size = 8, line_width = 0.5, l=20)  +
...
plot_1<-ggplot()+
     ...
     # geom_text(data = plot_data, aes(x=max(plot_data[,3])+20, y = plot_data[,2], group = plot_data[,1], label = plot_data[,1]), size = 3) +
      R_dateR_theme(text_size = 8, line_width = 0.5, l=20) + xlab("Year") + ylab("Number of samples") +
...
```{r problem_sample_plots, echo = FALSE, out.width = '100%', fig.out = 10, dpi = 120}
 ...
 for (i in 2:ncol(full_data)){
  ...
  ln_plot <- line_plot(combined, colnames(combined)[2], colnames(combined)[3], lag = 0, text_size = 8, line_width = 0.5, plot_line = 0.5)
