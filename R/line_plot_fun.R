...
line_plot<-function(the_data, series_1_nm, series_2_nm, lag = 0){
  ...
  if (class(the_data) != "data.frame"){
    warning("Error in line_plot(). Required data are not a data.frame")
    run <- FALSE

  # Check thre is enough data in the data.frame
  } else if (ncol(the_data)<=2){
    warning("Error in line_plot(). Insufficient data to calculate correlations")
    run <- FALSE

  } else if(class(series_1_nm) != "character"){
    run <- FALSE
    warning("Error in line_plot(). series_1_nm is not a character string.")

  } else if(series_1_nm %in% colnames(the_data) == FALSE){
    run <- FALSE
    warning("Error in line_plot(). series_1_nm can not be found in the loaded data.")

  } else if(class(series_2_nm) != "character"){
      run <- FALSE
      warning("Error in line_plot(). series_2_nm is not a character string.")

  } else if(series_2_nm %in% colnames(the_data) == FALSE){
      run <- FALSE
      warning("Error in line_plot(). series_2_nm can not be found in the loaded data.")

  } else if (class(lag) != "numeric"){
    lag <- 0
    warning("Error in line_plot(). lag should be a numeric integer.")
  }
