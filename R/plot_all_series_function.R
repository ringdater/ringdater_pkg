...
#' @param text_size A numeric to set the axis font size
#' @param line_width A numeric to set the width of the line
#' @param plot_line A numeric value to set the width of the lines being plotted
#' @export
#' @examples
...
plot_all_series<-function(aligned_data, text_size = 12, plot_line = 0.5, line_width = 1){
  ...
  if (ncol(aligned_data)<=2){
    ...
    plot1 <-  ggplot()+
      geom_line(data = new, aes(x = new[,1], y = new[,2], group = new[,3]), linewidth = plot_line, alpha=0.5, na.rm=TRUE) +
      R_dateR_theme(text_size = text_size, line_width = line_width, l=20) +
      geom_line(data=chron_dat, aes(x=chron_dat[,1], y=chron_dat[,2]), colour = "red", linewidth = plot_line, na.rm=TRUE) +
