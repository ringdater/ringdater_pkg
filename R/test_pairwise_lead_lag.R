...
lead_lag_analysis<-function(the_data, mode = 1, neg_lag = -20, pos_lag = 20, complete = TRUE, shiny = TRUE){
  if (class(neg_lag) != "numeric"){warning("Neg_lag needs to be a numeric integer")
    return(NULL)}
  if (class(pos_lag) != "numeric"){warning("Pos_lag needs to be a numeric integer")
    return(NULL)}
  if (class(complete) != "logical"){warning("complete = missing TRUE/FALSE")
    return(NULL)}
  if (class(shiny) != "logical"){warning("shiny missing TRUE/FALSE")
    return(NULL)}
  if (is.null(the_data)){
  if (shiny == TRUE){
    shinyalert("Oops!", "You need to load some data to run this analysis.", type = "error")
  } else if (shiny == FALSE){warning("Required data.frame is NULL - Enter a data.frame with at least three columns")}
    return(NULL)
  } else {
