...
ld_undated_chron<-function(files, series_names = NULL, pair_detrend = FALSE, detrending_select = 3, splinewindow = 21, shiny = FALSE){

  if (class(pair_detrend) != "logical"){
    stop("Warning in ld_undated_chron(): pair_detrend should be a TRUE/FALSE.")
    }
  if (detrending_select %in% c(1:7) == FALSE){
    stop("Error in ld_undated_chron(). detrending_select must be a numeric integer from 1 to 7.")
  }
  if (splinewindow %in% c(5:500) == FALSE){
    stop("Error in ld_undated_chron(). splinewindow must be a numeric integer from 5 to 500.")
  }
  if (class(pair_detrend) != "logical"){
    stop("Warning in ld_undated_chron(): powerT should be a TRUE/FALSE.")
  }
