...
#' @param text_size A numeric value to set the size of the font,
#' @param line_width A numeric value to set the width of the axis line.
#' @export
#' @examples
...
lead_lag_bar<-function(the_data, sample_1, sample_2, text_size = 12, line_width = 1){
  ...
  if(!master_names[1] %in% colnames(the_data)){
    ...
    plot1<-ggplot()+
      ...
      R_dateR_theme(text_size = text_size, line_width = line_width) +ylab("T_val") + xlab("Lag (Year)") +
